...
router.put("/usersettings", async (req, res) => {
  ...
  try {
    ...
    console.log(error)
    res.status(500).json({ message: "An error ocurred while updating the user's data" })
  }
});
...
router.post("/getUser", async (req, res, next) => {
  ...
  if (user==null){
    ...
    const bg = downloader({ fileId: `users/${username}/bg` });
    const [profilePic, bgPic] = await Promise.all(
      [profile, bg].map((e) => e.catch((error) => {
        console.log(error)
        return false
      }))
    );
...
router.put("/follow", async (req,res)=>{
  ...
  if (user.followers.find(e => e.equals(mongoose.Types.ObjectId(follower)))==undefined){
    ...
    user.followers = user.followers.filter(e => !e.equals(mongoose.Types.ObjectId(follower)));
    user.save().then( async ()=>{
      const otherUser = await User.findById(follower)
      otherUser.following = otherUser.following.filter(e =>!e.equals(mongoose.Types.ObjectId(profile)))
...
router.post("/notifications", async (req,res)=>{
  const promise = (e) =>{
    return new Promise(resolve =>{
      User.findOne({username:e.username}).then(user =>{
        ...
        cloudinary.api.resource(`users/${e.username}/profile`,(error,result)=>{
          if (result){
  ...
  const user = await User.findOne({username});
  if (user){
    const array = await Promise.all(user.notifications.notes.map(e => promise(e)))
    return res.status(200).json(array)
  }
  res.status(200).json([])
})

router.get("/123", async (req,res,next)=>{ 
  console.log("route here!")
  const user = await User.findOne({username:"@Mick129"})
  const note = "hello"
  next("A")
})

router.get("/321", (req,res)=>{
  
  res.status(200).json({somethign:"else"})
})

router.use((note,req,res,next) => {
  if (note){
